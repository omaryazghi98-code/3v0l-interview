...
try { await import('../data/interview-questions.js'); } catch (err) { console.error('Could not load expanded question bank:', err.message); }
...
const allRefs = [
  ...
  ...(D.english||[]).map(x=>({...x,type:'english'})), ...(Q||[]).map(x=>({...x,type:'question'}))
];
...
function relevance(text,x){
  ...
  const hay=tokenize([x.question,x.title,x.text,x.answer_fr,x.answer_en,x.company,...(x.variants||[]),...(x.tags||[]),...(x.phrases||[])].join(' '));
  const set=new Set(hay); let hits=q.reduce((n,w)=>n+(set.has(w)?1:0),0);
  const t=String(text||'').toLowerCase(); const title=String(x.title||x.question||'').toLowerCase();
...
function relevantRefs(text){
  const ranked=allRefs.map(x=>({x,hits:relevance(text,x)})).filter(r=>r.hits>0).sort((a,b)=>b.hits-a.hits).slice(0,8);
  if(ranked.length) return ranked.map(r=>({id:r.x.id,title:r.x.title||r.x.question||'',question:r.x.question||'',variants:r.x.variants||[],company:r.x.company||'',type:r.x.type,source_type:r.x.source_type||'',tags:(r.x.tags||[]).slice(0,8),text:r.x.text||'',phrases:r.x.phrases||[],answer_fr:r.x.answer_fr||'',answer_en:r.x.answer_en||'',short_fr:r.x.short_fr||'',short_en:r.x.short_en||'',risk_flags:r.x.risk_flags||[]}));
  return [];
}
function heuristic(text,context=[]){
  ...
  const rules=[
    ...
    {terms:['transfer','transfert','argent','recipient','destinataire'],ref:'Q-042',intent:'transfer_support',dir:['Vérifier le statut du transfert','Confirmer les informations nécessaires','Donner une prochaine étape claire','Ne pas inventer de délai']},
    {terms:['refund','remboursement','failed','échoué'],ref:'Q-043',intent:'refund_support',dir:['Vérifier le statut et le moyen de paiement','Expliquer uniquement le délai officiel','Suivre la procédure si le délai est dépassé']},
    ...
    {terms:['modsquad','attendance','assiduité','janvier 2024'],ref:'Q-014',intent:'career_accountability',dir:['Répondre directement','Prendre la responsabilité','Expliquer brièvement le contexte','Revenir à la stabilité actuelle']}
  ];
...
function referenceText(context){
  return (context||[]).map(x=>{
    const promptAnswer=ANSWER_LANGUAGE==='en' ? (x.answer_en||x.text||x.phrases?.join(' ')||'') : (x.answer_fr||x.text||x.phrases?.join(' ')||'');
    const label=x.question||x.title||x.id;
    const source=x.source_type?` | source=${x.source_type}`:'';
    const risks=x.risk_flags?.length?`\nRisk flags: ${x.risk_flags.join(' | ')}`:'';
    return `[${x.id}] ${label}${source}\n${promptAnswer}${risks}`;
  }).join('\n\n');
}
async function callProvider(name,body){
  ...
  const payload={model:cfg.model,messages:[
    {role:'system',content:`You are the realtime interview assistant for 3V0L. Answer in ${ANSWER_LANGUAGE==='en'?'English':'French'} only. Use ONLY the supplied interview references and Omar's verified experience. Treat source=verified_candidate and source=candidate_story as candidate facts; treat researched_role and scenario_roleplay as role/scenario guidance, not evidence of Omar's past employment. Never invent employers, tools, metrics, policies, timelines or responsibilities. Never mix employers. If a company policy is not supplied as verified/current role guidance, say Omar would verify it instead of guessing. Return valid JSON only with exactly these keys: intent (string), direction (array of 2-5 short cues), say_this (short natural answer, maximum 4 sentences), best_reference (one supplied id or null), confidence (0-1).`},
...
const server=http.createServer(async(req,res)=>{ try {
  ...
  if(url.pathname==='/health'&&req.method==='GET') return json(res,200,{ok:true,model:providerConfig(LLM_PRIMARY)?.model,llm:llmStatus(),remoteConfigured:true,audioProvider:AUDIO_PROVIDER,audioPort:AUDIO_PORT,sseUrl:NODE_STT_ENABLED?null:SSE_URL,questionBank:Q.length,state});
  ...
  if(url.pathname==='/state'&&req.method==='GET') return json(res,200,{...state,apis:{llm:llmStatus()},questionBank:Q.length});
  if(url.pathname==='/config'&&req.method==='GET') return json(res,200,{llm:llmStatus(),requestyConfigured:!!REQUESTY_KEY,cerebrasConfigured:!!CEREBRAS_KEY,questionBank:Q.length});
...
server.listen(PORT,HOST,()=>{const hosts=lanAddresses();console.log(`3V0L Copilot relay listening on http://${HOST}:${PORT}`);console.log(`3V0L Remote: ${hosts[0]?`http://${hosts[0]}:${PORT}/remote`:`http://localhost:${PORT}/remote`}`);console.log(`LLM primary: ${LLM_PRIMARY}`);console.log(`Cerebras: ${CEREBRAS_KEY?'configured':'not configured'}`);console.log(`Requesty: ${REQUESTY_KEY?'configured':'not configured'}`);console.log(`Expanded questions: ${Q.length}`);console.log(`Audio provider: ${AUDIO_PROVIDER}${NODE_STT_ENABLED?'':` (SSE: ${SSE_URL})`}`);});
