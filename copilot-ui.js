...
function shell(){
 ...
 rail.innerHTML=`<div class="copilot-head"><div><div class="eyebrow">3V0L · LIVE COPILOT</div><strong>AI interview rail</strong></div><div class="copilot-head-actions"><button class="copilot-icon" data-cp="mock" title="Inject a test question">◇</button><button class="copilot-icon" data-cp="toggle" title="Hide/show rail">${open?'→':'←'}</button></div></div>
 ...
 <section class="cp-section"><div class="cp-label">INTERVIEWER</div><div id="cpTranscript" class="cp-transcript">Waiting for audio / transcript…</div></section>
 ...
 <section class="cp-section cp-say"><div class="cp-label">SAY THIS</div><div id="cpSay" class="cp-say-text">—</div></section>
 ...
 <section class="cp-section"><div class="cp-label">CONFIDENCE</div><div class="cp-confidence"><span id="cpConfidenceBar"></span></div><div id="cpConfidence" class="cp-confidence-text">—</div></section>
...
function updateShell(){document.body.classList.toggle('copilot-open',open);const b=document.querySelector('[data-cp="toggle"]');if(b)b.textContent=open?'→':'←';}
...
async function poll(){try{const r=await fetch(API+'/state',{cache:'no-store'});if(!r.ok)throw new Error();const s=await r.json();if(s.lastEventAt&&s.lastEventAt!==lastAt){lastAt=s.lastEventAt;renderState(s)}document.getElementById('cpState').textContent=s.lastEventAt?'LISTENING':'READY';document.getElementById('cpModel').textContent=s.provider==='requesty'?'REQUESTY':s.provider==='local'?'LOCAL':'NO AI';running=true;document.querySelector('.cp-dot')?.classList.toggle('on',!!s.lastEventAt);if(s.apis&&JSON.stringify(apiConfig)!==JSON.stringify(s.apis)){} }catch{running=false;document.getElementById('cpState').textContent='BRIDGE OFF';document.getElementById('cpModel').textContent='START SERVER';document.querySelector('.cp-dot')?.classList.remove('on')}}
function renderState(s){document.getElementById('cpTranscript').textContent=s.transcript||'—';const dir=(s.direction||[]).map((x,i)=>`<div class="cp-direction-item"><b>${i+1}</b><span>${esc(x)}</span></div>`).join('');document.getElementById('cpDirection').innerHTML=dir||'<div class="cp-empty">No guidance.</div>';document.getElementById('cpSay').textContent=s.sayThis||'—';const c=Math.round((+s.confidence||0)*100);document.getElementById('cpConfidenceBar').style.width=c+'%';document.getElementById('cpConfidence').textContent=c?`${c}% match`:'—';const rb=document.getElementById('cpReference');const x=refs.find(r=>r.id===s.bestReference);rb.disabled=!x;rb.textContent=x?`${x.company?x.company+' · ':''}${x.title}`:'No strong reference yet';rb.dataset.ref=x?.id||'';rb.dataset.rtype=x?.id?.startsWith('call-')?'phone':x?.id?.startsWith('epic-')||x?.id?.startsWith('spotify-')||x?.id?.startsWith('beerwulf-')||x?.id?.startsWith('airalo-')?'story':'answer';}
...
async function toggleApi(key){const next=!apiConfig[key];try{const patch={[key]:next};apiConfig={...apiConfig,...await post('/config',patch)};updateApiLabels();const note=document.getElementById('cpApiNote');if(note){note.textContent=`${key[0].toUpperCase()+key.slice(1)} ${next?'enabled':'disabled'} · ${next?'provider may be used when active':'no new usage from this provider'}`;setTimeout(()=>note.textContent='Off = no new provider usage.',2200);}}catch(err){const note=document.getElementById('cpApiNote');if(note)note.textContent=`Control error: ${err.message}`;}}
...
function bind(){document.addEventListener('click',e=>{const el=e.target.closest('[data-cp]');if(el){const a=el.dataset.cp;if(a==='toggle'){open=!open;localStorage.setItem('3v0l-copilot-open',open?'1':'0');updateShell();return}if(a==='mock'||a==='test'){analyze('Parlez-moi d’une situation difficile avec un client.');return}if(a==='clear'){post('/clear',{}).catch(()=>{});return}if(a==='settings'){get('/config').then(c=>alert(`Requesty: ${c.requestyEnabled?'ON':'OFF'}\nDeepgram: ${c.deepgram?'ON':'OFF'}\nAzure Speech: ${c.azure?'ON':'OFF'}`)).catch(()=>alert('Copilot relay unavailable.'));return}}if(e.target.id==='cpReference')openReference()});poller=setInterval(poll,450);poll();window.addEventListener('resize',()=>document.body.classList.toggle('copilot-open',open));}
shell();
